...
class PrepareBaseModel:
    ...
    def save_model(path: Path, model: tf.keras.Model):
        try:
            ...
            logging.error(
                f"Exception occurred : Saving model interrupted due to {CustomException(e,sys)}"
            )
            raise CustomException(e, sys)
    ...
    def get_base_model(self):
        try:
            ...
            os.makedirs(
                os.path.dirname(self.prepare_base_model_config.base_model_filepath),
                exist_ok=True,
            )
            ...
            self.save_model(
                path=self.prepare_base_model_config.base_model_filepath,
                model=self.model,
            )
        except Exception as e:
            logging.error(
                f"Exception occurred : getting base model interrupted due to {CustomException(e,sys)}"
            )
            raise CustomException(e, sys)
    ...
    def _prepare_full_model(model, classes, freeze_all, freeze_till, learning_rate):
        try:
            ...
            logging.error(
                f"Exception occurred : preparing full model interrupted due to {CustomException(e,sys)}"
            )
            raise CustomException(e, sys)
    ...
    def update_base_model(self):
        try:
            ...
            os.makedirs(
                os.path.dirname(
                    self.prepare_base_model_config.updated_base_model_filepath
                ),
                exist_ok=True,
            )
            self.save_model(
                path=self.prepare_base_model_config.updated_base_model_filepath,
                model=self.full_model,
            )

        except Exception as e:
            logging.error(
                f"Exception occurred : Updating base model interrupted due to {CustomException(e,sys)}"
            )
            raise CustomException(e, sys)
